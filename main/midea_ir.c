 ...
 * 
 * 
 * Start condition: 8Pulsos of hight level and 8Pulsos of low level.
 * 
 *  START BYTE   Invert Byte |  FIRST BYTE      Invert byte       | LAST BYTE      Invert byte
 * [1011  0010] [0100  1101] | [ssss  ffff] ['s's's's  'f'f'f'f] | [tttt mmmm]  ['t't't't 'm'm'm'm]
 * Repetition	one after approx. 5 msec
 * stop bit: 0 -> 1 Pulso high level and 1 Pulso low level
 * 
 */
...
void midea_ir_send(MideaIR *ir)
{
    ...
    // ESP_LOGW("MIDEA", "ir.enabled: %d", ir.enabled);
    rmt_midea_ir_tx_send_raw_message(komeco/*dp.DataAsByteArray*/, 3);
}

void komeco_ir_on()
{
    uint8_t komeco[] = {0xB2, 0xBF, 0x50}; // Ligar COLL
    rmt_midea_ir_tx_send_raw_message(komeco/*dp.DataAsByteArray*/, 3);
}
void komeco_ir_off()
{
    uint8_t komeco[] = {0xB2, 0x7B, 0xE0}; // Desligar COOL
    rmt_midea_ir_tx_send_raw_message(komeco/*dp.DataAsByteArray*/, 3);
}

void midea_ir_move_deflector()
{
    uint8_t komeco[3] = {0xB2, 0x0F, 0xE0};

    rmt_midea_ir_tx_send_raw_message(komeco, 3);
}

void midea_ir_oscilate()
{
    uint8_t komeco[3] = {0xB2, 0x6B, 0xE0};
...
{
    uint8_t komeco[3] = {0xB5, 0xF5, 0xA2};
